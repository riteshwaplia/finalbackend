...
const sendWhatsAppMessage = async ({ to, type, message, phoneNumberId, accessToken, facebookUrl, graphVersion }) => {
    ...
    if (!phoneNumberId || !accessToken || !facebookUrl || !graphVersion) {
        return { success: false, error: resMessage.Meta_API_credentials_not_configured + " (Missing Phone ID, Access Token, Facebook URL, or Graph Version)." };
    }
...
exports.sendMessageService = async (req) => {
    ...
    if (!to || !type || !message) {
        return {
            ...
            success: false,
            message: resMessage.Missing_required_fields + " (to, type, and message are required for single send)."
        };
    }
    ...
    if (!project.isWhatsappVerified || !project.metaPhoneNumberID) {
        return {
            status: statusCode.BAD_REQUEST,
            success: false,
            message: resMessage.Project_whatsapp_number_not_configured + " (WhatsApp number not verified or Meta Phone Number ID is missing for this project. Please link and verify a WhatsApp number to this project)."
        };
    }
    ...
    if (!businessProfile || !businessProfile.metaAccessToken || !businessProfile.metaBusinessId || !businessProfile.facebookUrl || !businessProfile.graphVersion) {
        return {
            status: statusCode.BAD_REQUEST,
            success: false,
            message: resMessage.Meta_API_credentials_not_configured + " for the linked Business Profile. Please update your Business Profile with Access Token, WABA ID, Facebook URL, and Graph Version."
        };
    }
    ...
    try {
        const sendResult = await sendWhatsAppMessage({ to, type, message, phoneNumberId, accessToken, facebookUrl, graphVersion });
        ...
        const messageData = new Message({
            ...
            templateLanguage: type === 'template' ? message.language?.code : undefined,
        });
        ...
        return {
            status: sendResult.success ? statusCode.OK : statusCode.INTERNAL_SERVER_ERROR,
            success: sendResult.success,
            message: sendResult.success ? resMessage.Message_sent_successfully : (sendResult.error?.message || resMessage.Message_send_failed),
            data: {
...
exports.sendBulkMessageService = async (req) => {
    ...
    let templateComponents = message.components;
    let templateLanguageCode = message.language?.code || 'en_US';
...
exports.getBulkSendJobDetailsService = async (req) => {
    ...
    if (!bulkSendJobId) {
        return {
            status: statusCode.BAD_REQUEST,
            success: false,
            message: resMessage.Missing_required_fields + " (bulkSendJobId is required)."
        };
    }
    ...
    try {
        ...
        const messagesInJob = await Message.find({ bulkSendJobId: bulkSendJobId }).sort({ createdAt: 1 });
...
exports.uploadMedia = async (req) => {
    ...
    try {
        ...
        if (!project || !project.metaPhoneNumberID || !project.businessProfileId?.metaAccessToken) {
            return {
                status: 400,
                success: false,
